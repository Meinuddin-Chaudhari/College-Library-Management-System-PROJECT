...
<body>
	...
	<main>
		...
		<table class="center">
			...
			<c:forEach var="data" items="${requestScope.REQUESTSEARCHBOOKLIST}">
			<tr>
				<td>${data.bookId}</td>
				<td>${data.title}</td>
				<td>${data.author}</td>
				<td>${data.category}</td>
				<td>
							<form action="addRequestedBook.jlc">
								<input type="hidden" name="bookId" value="${data.bookId}"/>
								<input type="hidden" name="studentId" value="${sessionScope.STUDENTREGISTERTO.studentId}"/>
